          ...
          <v-row>
            <v-row >
              <v-col sm="12" xl="6">
                ...
                <v-col cols="12" >
                  <v-select :items="formatedCategories" v-model="selectedCategory" label="Catégorie" />
                </v-col>
                <v-col  cols="12" >
                  <v-text-field v-model="city" :rules="cityRules"  :error-messages="errors" label="Ville" required />
                </v-col>
              </v-col>
              <v-col class="d-flex justify-center" cols="12" sm="12" xl="6">
                <v-date-picker v-model="date" locale="FR-fr" :min="new Date().toISOString().substr(0, 10)" class="mt-4"></v-date-picker>
              </v-col>
            </v-row>
          </v-row>
          <v-row>
            <v-col cols="12">
              ...
              <v-container>
                <v-row>
                  <v-col v-for="item of availableItems" :key="item.id" class="d-flex child-flex" cols="6" sm="4">
                    <ItemCard :item="item" />
                  </v-col>
                </v-row>
              </v-container>
...
export default {
  ...
  data() {
    return {
      ...
      cityRules: [
        v => !!v || 'Une nom de ville est requis'
      ],
      model: [],
      categories: [],
  ...
  watch: {
    "city": { 
      async handler(val) {
        ...
        const foundCity = cities.data.find((city) => city.nom.toUpperCase() === val.toUpperCase().trim());
        if (foundCity){
          this.errors = [];
        }
      }
